...
int main()
{
	...
	_asm{
		...
		MOV EAX, num_x;
		MOV EBX, num_y;
		MOV EDX, 0;//被除数为EDX:EAX，高32位置零
		DIV EBX;		//除运算：商在EAX，余数在EDX
		PUSH EDX;		//保护此时的EDX值
		
		LEA EDI, str_quotient;
		CALL CONVERT_INT_TO_STR;	//EAX中为商，此处转商为字符串
		
		POP EDX;		//恢复除法运算结果的EDX值
		MOV EAX, EDX;
		LEA EDI, str_remainder;
		CALL CONVERT_INT_TO_STR;	//EAX中为余，此处转余为字符串
	}

	printf("该字符对应的十进制数字x = %d  y = %d\n",num_x,num_y);
	printf("x÷y 之商：%s\n",str_quotient);
	printf("x÷y 之余：%s\n",str_remainder);
