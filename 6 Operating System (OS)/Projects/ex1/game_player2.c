...
{
    printf("石头剪刀布游戏 - roshambo_player2\n");
    ...
    for(int i=0; i<100; i++)
    {
        ...
        snd_msg.mtype = 2;      // 玩家id, default msg type(player-1)
        // srand(time(0));
        ...
        printf("player2_id = %d,  %s\n", ipc_id_player, snd_msg.mtext);
        ...
        while(1)
        {
            ...
            if(ret == -1) {
                ...
                if(rcv_msg.mtype != 2){    // 取错了，还回去，继续循环等待
